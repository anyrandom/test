...
st.markdown(
    ...
    .embeddedAppMetaInfoBar_container__DxxL1{background-color: #fff !important;height:100px!important}
...
for message in st.session_state.messages:
    if message["role"] == "assistant":
        with st.chat_message(message["role"], avatar=barista_logo):
            st.write(message["content"])
    else:
        with st.chat_message(message["role"]):
            st.write(message["content"])

# enable_user_voice_search = False
# with st.sidebar:
...
#     prompt = st.chat_input(voice_input)
# else:
...
if not audio.empty():
    ...
    voice_input = handle_voice_input(audio)
    prompt = st.chat_input(voice_input)
    #
